# Skills

Skills are special abilities and passive bonuses your character can have. They come from three sources:

- **Class** — your starting class grants one skill immediately and unlocks more to purchase.
- **Background** — your chosen background grants one starting skill.
- **Items** — some weapons grant a skill while equipped (e.g. the Flamethrower).

You can spend class points to buy additional skills from your class's skill list. Open the upgrade screen with **Shift+C**.

---

## Combat Skills

### Ranged

| Skill | Effect |
|---|---|
| Sharpshooter | +5 CS |
| Ranged Combat Specialist | +5 CS |
| Deadshot | +10 CS on aimed shots; targeted aimed shots hit the chosen body part 90% of the time |

### Melee

| Skill | Effect |
|---|---|
| Combat Specialist | +5 HTCS |
| Brawler | +2 Natural SP, +1 PH |
| Hands Only | Unarmed attacks deal more damage and cost half energy |
| Roundhouse Kick | Strike all adjacent enemies at once (Key: K) |
| Shove | Push the entity directly in front of you one tile back (Key: V) |

---

## Defensive Skills

| Skill | Effect |
|---|---|
| Thick Skin | +2 Natural SP (absorbs incoming damage) |
| Noncombustible | Take half damage from fire |
| Acid Proof | Take half damage from acid |

---

## Movement Skills

| Skill | Effect |
|---|---|
| Sprinter | Move faster than normal |
| Nimble | +5 AG |
| Heavy Footed | Move slower than normal |

---

## Special Attack Skills

These skills grant a key-bound special attack.

| Skill | Key | Effect |
|---|---|---|
| Flamethrower (item) | C | Fire a cone of flame (depth 3, widening spread) |
| Acid Spray | C | Fire a cone of acid (depth 5, straight line) |
| Poisonous Bite | B | Bite the enemy in front of you. On a failed Cool Check, they are poisoned for 5 turns. |
| Paralyzing Vinewhip | V | Strike the enemy in front of you. On a failed Cool Check, they are slowed for 4 turns. |

---

## Overgrowth Skills

Skills related to the spreading of organic overgrowth.

| Skill | Key | Effect |
|---|---|---|
| Spread Overgrowth | G | Spread overgrowth to tiles within radius 4. Each eligible neighbor has a 40% chance to become overgrown. Crosses Z-levels through stairs. Costs 400 energy. |
| Trail Overgrowth | — | Passive. Each tile you move onto has a 10% chance of becoming overgrown. |
| Plant Food | — | Passive. While standing on an overgrown tile, gain 25% more energy per turn. |

---

## Class Passive Skills

These skills are granted automatically by specific classes and act as flags for game mechanics.

| Skill | Granted By | Effect |
|---|---|---|
| Stellar Cartography | Navigator | Starts the game with the entire floor revealed on the minimap |
| Officer Clearance | Officer | Grants access to most secured doors aboard the ship |
| Systems Expert | Engineer | Improves ability to diagnose and repair broken ship systems and computers |

---

## Other Skills

| Skill | Effect |
|---|---|
| Brainy | +1 MA |
| Small | Move through maintenance tunnels at full speed |
